...
function bflowStep4(o) {
  ...
  for (var i = 0; i < tp; i++) {
    ...
    h += '<select class="fi fi-sel" id="bf-ttl' + i + '"><option value="mr">' + t('pf_mr') + '</option><option value="ms">' + t('pf_ms') + '</option></select></div>';
...
function showConfirmation(bookingData, opts) {
  ...
  html += '<div style="text-align:center;margin-top:16px;font-size:11px;color:var(--tx3)">Eine Bestätigung wurde an deine E-Mail-Adresse gesendet.</div>';
...
function renderReferralPanel() {
  ...
  body.innerHTML =
    '<div style="background:linear-gradient(135deg,var(--teal),var(--teal2));border-radius:18px;padding:20px;color:#fff;margin-bottom:16px;position:relative;overflow:hidden">' +
      ...
      '<div style="font-size:13px;opacity:.9;line-height:1.5">' + tL('Sobald dein Freund seinen Flug antritt, erhaltet ihr beide 10€ Guthaben — automatisch.','As soon as your friend boards their flight, you both instantly get €10 credit.','بمجرد صعود صديقك للطائرة، يحصل كلاكما فورًا على 10€ رصيد.') + '</div>' +
    '</div>' +
