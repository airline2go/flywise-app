 ...
 *
 * This is intentionally input-level rather than an acS wrapper. The existing
 * app input dispatcher is the real entry point (data-inp-fn=handleFromInput /
 * handleToInput); wrapping an internal/global function is brittle and can be
 * bypassed by scope/order changes. Capture-phase interception makes the local
 * resolver authoritative whenever AP has a result and leaves the existing
 * network fallback untouched when it does not.
 *
 * AP rows are the source of truth. Every searchable string in a row is indexed
 * and the selected result always commits its canonical three-letter IATA code.
 * No raw user spelling is sent to the flight search.
 */
(function () {
  'use strict';
  ...
  function names(row) {
    ...
    for (var i = 0; i < row.length; i++) {
      if (typeof row[i] === 'string' && row[i].trim()) out.push(fold(row[i]));
    }
  ...
  function build() {
    ...
    for (var i = 0; i < window.AP.length; i++) {
      ...
      entries.push({ row: row, code: String(row[0]).toUpperCase(), vals: vals });
    }
  ...
  function score(entry, q) {
    var fq = fold(q), best = 0;
    var qt = fq.split(' ').filter(Boolean);
    for (var i = 0; i < entry.vals.length; i++) {
      ...
      else if (qt.length && qt.every(function (t) {
        return vt.some(function (x) { return x.indexOf(t) === 0; });
  ...
  function resolve(q) {
    ...
    for (var i = 0; i < data.entries.length; i++) {
      var e = data.entries[i], s = score(e, q);
      if (s) out.push({ entry: e, score: s });
    }
    out.sort(function (a, b) {
      return b.score - a.score || a.entry.code.localeCompare(b.entry.code);
    });
  ...
  function commit(side, row) {
    ...
    /* app.js is a classic script and keeps these canonical values globally. */
    window[side + 'I'] = code;
    window[side + 'C'] = c;
    ...
    if (input) {
      input.dispatchEvent(new CustomEvent('fw-place-selected', {
        bubbles: true,
        detail: { side: side, iata: code, city: c, airport: airport(row) }
      }));
    }
  }
  ...
  function onInput(ev) {
    ...
    if (!results.length) return; /* preserve the existing fallback */

    /* Prevent handleFromInput/handleToInput from reaching the network path. */
    ev.stopPropagation();
