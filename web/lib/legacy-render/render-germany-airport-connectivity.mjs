...
export async function renderGermanyAirportConnectivityHtml(lang = 'de') {
  ...
  const mainContent = `<main id="de-air-main"><div class="de-air-wrap">${breadcrumb}<h1>${copy.h1}</h1><p class="de-air-intro">${copy.intro}</p><div class="de-air-snapshot">${copy.snapshot} · ${copy.generated}: ${snapshotDate}</div><div class="de-air-scope">${copy.scopeNote}</div><div class="de-air-kpis">${stats}</div><section class="de-air-section"><h2>${copy.highlights}</h2><div class="de-air-highlights">${highlights}</div></section><div class="de-air-grid"><section class="de-air-section"><h2>${copy.topAirports}</h2><p class="de-air-note">${copy.note}</p><ol class="de-air-list">${airportRows}</ol></section><section class="de-air-section"><h2>${copy.topCountries}</h2><p class="de-air-note">${copy.note}</p><ol class="de-air-list">${countryRows}</ol></section></div><section class="de-air-section"><h2>${copy.topCorridors}</h2><p class="de-air-note">${copy.note}</p><ol class="de-air-list">${corridorRows}</ol></section><section class="de-air-section"><h2>${copy.methodology}</h2><div class="de-air-method">${copy.methodologyText}<div class="de-air-links"><a class="de-air-link" href="${networkHref}">${copy.openAsset}</a><a class="de-air-link" href="${csvHref}">${copy.download}</a></div></div></section><section class="de-air-section"><h2>${copy.linkedAsset}</h2><div class="de-air-method">${copy.linkedAssetText}</div></section></div></main>`;
  const csvDistribution = { '@type': 'DataDownload', contentUrl: `https://airpiv.com${csvHref}`, encodingFormat: 'text/csv', name: `${copy.title} CSV` };
  const datasetSchema = { '@context': 'https://schema.org', '@type': 'Dataset', name: copy.title, description: copy.description, url: canonical, inLanguage: activeLang === 'de' ? 'de-DE' : 'en-GB', dateModified: snapshotDate, creator: { '@type': 'Organization', name: 'Airpiv', url: 'https://airpiv.com', logo: 'https://airpiv.com/apple-touch-icon.png' }, publisher: { '@type': 'Organization', name: 'Airpiv', url: 'https://airpiv.com', logo: 'https://airpiv.com/apple-touch-icon.png' }, distribution: [csvDistribution], variableMeasured: ['Published Germany-origin route records', 'International Germany-origin route records', 'German origin airports', 'Recorded destination airports', 'International destination countries', 'Directional airport-to-country corridors'] };
