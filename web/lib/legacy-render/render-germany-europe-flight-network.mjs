...
// into a Europe view. The underlying route rows remain untouched.
const EUROPE = new Set([
  'AL','AD','AM','AT','AZ','BY','BE','BA','BG','HR','CY','CZ','DK','EE','FI','FR','GE','DE','GR','HU','IS','IE','IT','XK','LV','LI','LT','LU','MT','MD','MC','ME','MK','NL','NO','PL','PT','RO','RU','SM','RS','SK','SI','ES','SE','CH','TR','UA','GB','VA',
]);
...
const COPY = {
  de: {
    ...
    intro: 'Dieser Datenindex betrachtet ausschließlich veröffentlichte Airpiv-Routen mit Deutschland als Abflugland und europäischen Zielmärkten. Er zeigt die im aktuellen Routenkatalog erfassten Abflüge, Zielflughäfen, Zielländer und deutschen Airport→Country-Korridore.',
    ...
    methodology: 'Methodik & Grenzen', methodologyText: 'Die Seite filtert veröffentlichte Route-Records auf origin_country = DE und auf eine fest definierte Menge europäischer ISO-2-Ländercodes. Zielzahlen zählen unterschiedliche destination_iata-Werte; Korridore sind gerichtet. Fehlende Länder- oder IATA-Werte werden nicht ergänzt.',
    related: 'Verwandte Datenindizes', network: 'Länder- und Netzwerkindex öffnen', germany: 'Deutschland-Konnektivität öffnen', flightData: 'Flugdaten-Index öffnen', records: 'Records', destinations: 'Ziele', airports: 'Zielflughäfen',
  },
  en: {
    ...
    intro: 'This data index uses only published Airpiv routes with Germany as the origin country and European destination markets. It shows German departure airports, recorded destination airports and countries, and directional Germany-to-Europe airport corridors represented in the current route catalogue.',
    ...
    methodology: 'Methodology & limitations', methodologyText: 'This page filters published route records to origin_country = DE and a fixed set of European ISO-2 country codes. Destination counts use distinct destination_iata values; corridors are directional. Missing country or IATA values are not inferred.',
    related: 'Related data indexes', network: 'Open country and network index', germany: 'Open Germany connectivity report', flightData: 'Open flight-data index', records: 'records', destinations: 'destinations', airports: 'airports',
  },
};
