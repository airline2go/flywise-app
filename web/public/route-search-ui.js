(function () {
  ...
  function searchAirports(side, q) {
    var box = acBox(side); if (!box || q.length < 1) { if (box) box.classList.remove('open'); return; }
    ...
    timers[side] = setTimeout(function () {
      ...
      run.then(function () {
        return fetch('/search/airports?q=' + encodeURIComponent(q), { headers: { Accept: 'application/json' } });
  ...
  function setSide(side, code, name) {
    ...
    el.value = name || code; el.setAttribute('data-route-code', code); closeDrops();
  }
  function swap() {
    var f = input('from'), t = input('to'); if (!f || !t) return;
    var fc = f.getAttribute('data-route-code') || '', tc = t.getAttribute('data-route-code') || '';
    var fv = f.value, tv = t.value; setSide('from', tc, tv); setSide('to', fc, fv);
  }
  ...
  function updatePaxButton() {
    ...
    var c = b.closest('[data-route-search]'); var tr = c && c.querySelector('[data-route-trip]');
    ...
    if (tr && tr.value === 'ow') { var rw = document.querySelector('[data-route-return-wrap]'); if (rw) rw.classList.add('disabled'); }
  }
  function updateCounters() {
    Object.keys(state).forEach(function (k) { var el = document.querySelector('[data-route-pax-count="' + k + '"]'); if (el) el.textContent = state[k]; });
    updatePaxButton();
  }
  function validateDateRelation(f) {
    var dep = f.querySelector('[name="depart"]'); var ret = f.querySelector('[name="ret"]'); var trip = f.querySelector('[data-route-trip]');
    var rw = document.querySelector('[data-route-return-wrap]');
  ...
  function submit(ev) {
    ...
    if (!fc || !tc || fc === tc) return setError('Please choose a valid origin and destination.');
    ...
    p.set('depart', dep.value); p.set('trip', trip ? trip.value : 'rr');
  ...
  function init() {
    ...
    var f = form(); if (!f) return;
    var c = cfg(f); var from = input('from'), to = input('to');
    if (from) from.setAttribute('data-route-code', c.from || ''); if (to) to.setAttribute('data-route-code', c.to || '');
    var dep = f.querySelector('[name="depart"]'), ret = f.querySelector('[name="ret"]');
    var today = new Date().toISOString().slice(0, 10); if (dep) dep.min = today; if (ret) ret.min = today;
    f.addEventListener('submit', submit);
    f.addEventListener('input', function (ev) { var el = ev.target; if (el && el.getAttribute('data-route-side')) searchAirports(el.getAttribute('data-route-side'), el.value.trim()); });
    ...
    f.addEventListener('click', function (ev) {
      var pick = ev.target.closest('[data-route-pick]');
      if (pick) { ev.preventDefault(); setSide(pick.getAttribute('data-route-side'), pick.getAttribute('data-route-pick'), pick.getAttribute('data-route-name')); return; }
      var action = ev.target.closest('[data-route-action]');
      if (action) {
        ev.preventDefault(); var a = action.getAttribute('data-route-action');
        if (a === 'swap') swap();
        if (a === 'pax') { var m = document.querySelector('[data-route-pax-menu]'); if (m) { m.classList.toggle('open'); action.setAttribute('aria-expanded', m.classList.contains('open') ? 'true' : 'false'); } }
        return;
      }
      var pax = ev.target.closest('[data-route-pax]');
      if (pax) { ev.preventDefault(); var k = pax.getAttribute('data-route-pax'), d = Number(pax.getAttribute('data-route-delta') || 0), next = state[k] + d; if (k === 'adult') next = Math.max(1, Math.min(9, next)); else next = Math.max(0, Math.min(8, next)); if (k === 'infant') next = Math.min(next, state.adult); state[k] = next; updateCounters(); return; }
    });
